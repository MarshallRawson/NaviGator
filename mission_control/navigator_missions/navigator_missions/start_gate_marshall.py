...
class StartGateMarshall(Navigator):
	...
	def go_around_black_totem(self):
		locations = yield self.get_bouy_go_round_target()	
		waypoint = np.append((((start_pose-locations[0])/np.linalg.norm(start_pose-locations[0]))*TOTEM_MARGIN)+locations[0], 0)
		yield self.move.set_position(waypoint).look_at(locations[0]).go()
		circle = self.move.circle_point([location[0][0], location[0][1], 0], "ccw", 1)
		yield self.move.yaw_right(math.pi*2).go()
        @txros.util.cancellableInlineCallbacks
